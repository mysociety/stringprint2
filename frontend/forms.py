...
class AdminImageWidget(AdminFileWidget):
    def render(self, name, value, attrs=None):
        ...
        if value and getattr(value, "url", None):
            image_url = value.url
            file_name = str(value)
            output.append(' <a href="%s" target="_blank"><img src="%s" alt="%s" width=300 /></a>' %
...
class PageHeaderImageForm(forms.ModelForm):
    ...
    def save(self, article, commit=True, files=None):
        ...
        if commit:
            if hasattr(ins, "prior") == False or ins.prior != prior or ins.prior_h != prior_h:
                ins.queue_tiny = True
...
class ArticlePublishForm(forms.Form):
    ...
    publish_url = forms.CharField(label="Final URL",
                                  help_text="Where will this document be visible on the web? (used for social sharing).",
                                  required=True)
    publish_date = forms.DateField(label="Publish Date",
...
class KindleEditForm(forms.ModelForm):
    book_cover = forms.ImageField(label="Kindle Book Cover",
                                  required=True,
                                  widget=AdminImageWidget,)
...
class OrgAmendForm(forms.ModelForm):
    name = forms.CharField(label="Organisation Name",
                           help_text="Used in citation construction.",
                           required=True)
    twitter = forms.CharField(label="Organisation twitter handle",
                              help_text="This associated twitter card social sharing\
                            to the correct account")
    ga_code = forms.CharField(label="Google Analytics Tracking ID",
...
class ReuploadWordForm(HoneyPotForm):
    ...
    file = forms.FileField(label="Upload a Word file (optional)",
                           required=False,
...
class NewDocumentForm(HoneyPotForm):
    ...
    title = forms.CharField(label="Document Title",
                            help_text="Don't worry, this can be changed later.")
    file = forms.FileField(label="Upload a Word file (optional)",
                           required=False,
                           validators=[validate_file_extension])
