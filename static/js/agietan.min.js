hide_state=true;$(".noscript-anchor").remove()
...
$('#master_expand').on('click touchend',function(event){event.preventDefault();$(".extended-row").toggle();$(".extended").fadeToggle();hide_state=!hide_state;if(hide_state==true){$('#master_expand_link').text("Show All Notes");$(".expand-link").slideDown();}else{$('#master_expand_link').text("Hide All Notes");$(".expand-link").slideUp();};});function changeSelected(id,title,nohash){if(title){$("#mobile-title").text(title);}else{short_name=$("body").attr("title")
...
year=$("body").attr("year")
org=$("body").attr("org")
if(para_tag!="None"){ele_name="Paragraph"
ele_id=para_tag}else{ele_name="Element"
ele_id=$(this).attr("id")}
swal({title:dialog_title,text:'Link: <a href="'+link+'">'+title+': '+ele_name+' '+ele_id+'</a>'+
