var hide_state=true;var page_loaded=false;var current_section="";$(".noscript-anchor").remove()
if(!String.prototype.startsWith){String.prototype.startsWith=function(searchString,position){position=position||0;return this.indexOf(searchString,position)===position;};}
function send_ga(action,value){label=$("meta[name=slug]").attr("content")
value=typeof value!=='undefined'?value:null;if(typeof ga==='function'){ga('send','event','StringPrint',action,label,value)}}
$('#master_expand').on('click touchend',function(event){event.preventDefault();$(".extended-row").toggle();$(".extended").fadeToggle();hide_state=!hide_state;if(hide_state==true){$('#master_expand_link').text("Show All Notes");$(".expand-link").slideDown();}else{$('#master_expand_link').text("Hide All Notes");$(".expand-link").slideUp();};});function changeSelected(id,title,nohash){if(title.length>0){$("#mobile-title").text(title);}else{title=$("meta[name=short_title]").attr("content");$("#mobile-title").text(title);}
...
adjustMenus(id);paragraph_id=$("#"+id+".linkable").attr("first_graf");set_mobile_link(paragraph_id);}
$("a.nav-link").on("click",function(){send_ga("Mobile Menu Click",$(this).attr('href'));})
$("a.search_link").on("click",function(){send_ga("Search");})
$("a.alt_link").on("click",function(){send_ga("Alt Format - "+$(this).attr('type'));})
$("a.top-nav-item").on("mouseup",function(){send_ga("Top Nav Click",$(this).attr('href'));id=$(this).attr('section');title=$("#"+id+".section-anchor").text()
changeSelected(id,title,true);})
var waypoints=$('.section-anchor').waypoint(function(direction){if($(this.element).is(':visible')==false){return 0;}
if(direction=="down"){id=$(this.element).attr('id');title=$(this.element).text()}else{id=$(this.element).attr('prev');title=$("#"+id+".section-anchor").text()};if(page_loaded){changeSelected(id,title);}},{offset:function(){if($(this.element).is(':visible')==true){return $(this.element).height()+60;}else{return 0}},continuous:false});function scrollToView(element){var offset=element.offset().top+20;if(!element.is(":visible")){element.css({"visibility":"hidden"}).show();var offset=element.offset().top;element.css({"visibility":"","display":""});}
...
$(".cite-link").not(".manual_paragraph").hide();$('#'+id+".cite-link").show();set_mobile_link(id);})
var last_id=1
var default_past=1
function set_mobile_link(id){item=$("#"+id+".cite-link");link_ref=item.attr("href");para_tag=item.attr("para_tag");mobile_link=$(".mobile-header-link");mobile_link.attr("href",link_ref);mobile_link.attr("para_tag",para_tag);mobile_link.prop("id",id);}
...
$('#'+para_id+".extended-row").show();$('#'+para_id+".extended").show();$('#'+para_id+".catchup-link").show();$('#'+para_id+".cite-link").addClass("manual_paragraph");}
...
currently_moving=false;set_mobile_link(para_id);if(page_loaded==false){id=target.attr('parent')
...
$('a[href*="#"]:not([href="#"])').on('click touchend',function(){if(dragging==false&&location.pathname.replace(/^\//,'')==this.pathname.replace(/^\//,'')&&location.hostname==this.hostname&&location.pathname==this.pathname&&$(this).hasClass('cite-copy')==false){var splits=this.hash.split(".")
...
function citeBox(dialog_title,link,para_id,para_tag){title=$("meta[name=short_title]").attr("content")
...
if(para_tag!=""){ele_name="Paragraph";ele_id=para_tag;}else{ele_name="Paragraph";ele_id=para_id;};swal({title:dialog_title,text:'Link: <a href="'+link+'">'+title+current_section_name+', '+ele_name+' '+ele_id+'</a>'+
...
clipboard.copy(link).then(function(){citeBox("Link added to Clipboard!",link,para_id,para_tag);},function(err){citeBox("Copy Link From Below!",link,para_id,para_tag);})
...
$(window).on("load",function(){hide_state=$("meta[name=hide_state]").attr("content")
if(hide_state=="true"){hide_state=true}else if(hide_state=="false"){hide_state=false}
if(hide_state==true){$(".extended").hide();$(".extended-row").hide();$('#master_expand_link').text("Show All Notes");}else{$('#master_expand_link').text("Hide All Notes");$(".expand-link").hide();};if(window.location.hash){var hash=window.location.hash;splits=hash.split(".");if(splits.length>1){page_loaded=true;return gotoHash(hash,true);}}
page_loaded=true;});function followHash(){var splits=location.hash.split(".")
if(splits.length>1){gotoHash(location.hash);}}
function window_resize(){adjustMenus();Waypoint.refreshAll()}
window.onresize=window_resize;window.onhashchange=followHash;window_resize()
set_mobile_link(1)