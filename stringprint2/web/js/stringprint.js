...
function send_ga(action, value) {
    ...
    if (typeof gtag === 'function') {
        gtag('event', 'stringprint_event', { "action": action, "label": label, "value": value })
    }
}
...
function local_link_handler() {
    ...
    if (dragging == false
        ...
        if (splits[0].startsWith("#ref")) {
            ...
            new_loc = $('a[name="' + new_hash + '"]').offset().top - 30;
            $("html,body").animate({ scrollTop: new_loc }, 1);
            return false
        }
...
//grab local # links
$('a[href*="#"]:not([href="#"])').on('click touchend', local_link_handler);
$('#menu-prev').on('click touchend', local_link_handler);
$('#menu-next').on('click touchend', local_link_handler);
...
$('.nav-link').not('.dropdown-toggle').on('click touchend',
    function () {
        ...
        dest = this.href.split("#")[0]

        if (this.hash[0] && current == dest) {
            new_loc = $('a[name="' + this.hash.slice(1) + '"]').offset().top - 30;
            $("html,body").animate({ scrollTop: new_loc }, 1);
